...
controller_interface::return_type EurobotDriveController::update(const rclcpp::Time & time, const rclcpp::Duration & period)
{
...
controller_interface::CallbackReturn EurobotDriveController::on_activate(const rclcpp_lifecycle::State &)
{
    ...
    fl_wheel_ = get_wheel(roue_AvG_joint_name_);
    fr_wheel_ = get_wheel(roue_AvD_joint_name_);
    rl_wheel_ = get_wheel(roue_ArG_joint_name_);
    rr_wheel_ = get_wheel(roue_ArD_joint_name_);
