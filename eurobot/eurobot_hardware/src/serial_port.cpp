...
#include "include_hardware/serial_port.hpp"
...
EurobotSerialPort::EurobotSerialPort()
    : serial_port_(-1)
    , rx_frame_length_(0)
...
return_type EurobotSerialPort::open(const std::string & port_name)
{
...
return_type EurobotSerialPort::close()
{
    if (is_open()) {
        ::close(serial_port_);
        serial_port_ = -1;
...
return_type EurobotSerialPort::read_frames(std::vector<SerialHdlcFrame>& frames)
{
...
return_type EurobotSerialPort::write_frame(const uint8_t* data, size_t size)
{
    if (!is_open()) {
        return return_type::ERROR;
    }
...
bool EurobotSerialPort::is_open() const
{
    return serial_port_ >= 0;
}

void EurobotSerialPort::encode_byte(uint8_t data)
{
...
void EurobotSerialPort::decode_byte(uint8_t data, std::vector<SerialHdlcFrame>& frames)
{
    ...
    if (rx_frame_length_ == Eurobot_SERIAL_SERIAL_FRAME_MAX_SIZE) {
        rx_frame_length_ = 0;
        rx_frame_crc_ = HDLC_CRC_INIT_VALUE;
    }
}

uint16_t EurobotSerialPort::crc_update(uint16_t crc, uint8_t data)
{
